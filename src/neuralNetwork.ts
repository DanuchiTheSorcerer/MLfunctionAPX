...
export class NeuralNetwork {
    ...
    for (let i = 0; i < layerSizes.length - 1; i++) {
      ...
      this.weights.push(
        new Matrix(layerSizes[i + 1], layerSizes[i], () =>
          Math.random() * Math.sqrt(2 / fanIn) * (Math.random() > 0.5 ? 1 : -1) 
        )
      );
  ...
  private sigmoid(input: Vector): Vector {
    return new Vector(
      ...
      (i: number) => Math.tanh(input.components[i]) 
    );
}

private sigmoidPrime(input: Vector): Vector {
    let tanhVec = this.sigmoid(input); 
    let oneVec = new Vector(input.components.length, () => 1);
    return oneVec.subtract(tanhVec.mult(tanhVec));
}
